...
def run():
    ...
    # --- Bundle editable copies of songs.csv, political_events.csv, lexicons/ ---
    bundle_editable_songs()
    bundle_editable_events()
    bundle_editable_lexicons()
    write_editing_instructions()
...
def bundle_editable_songs() -> None:
    ...
    print(f"Bundled {dst} for collaborator hand-off")


def bundle_editable_events() -> None:
    if not EVENTS_CSV.exists():
        print(f"Warning: {EVENTS_CSV} not found — skipping editable political_events.csv bundle")
        return
    dst = DASHBOARD_DIR / "political_events.csv"
    shutil.copy2(EVENTS_CSV, dst)
    print(f"Bundled {dst} for collaborator hand-off")


def bundle_editable_lexicons() -> None:
    if not LEXICONS_DIR.exists():
        print(f"Warning: {LEXICONS_DIR} not found — skipping editable lexicons bundle")
        return
    dst_dir = DASHBOARD_DIR / "lexicons"
    dst_dir.mkdir(parents=True, exist_ok=True)
    for f in LEXICONS_DIR.glob("*.txt"):
        shutil.copy2(f, dst_dir / f.name)
    print(f"Bundled {dst_dir} for collaborator hand-off")


def write_editing_instructions() -> None:
    (DASHBOARD_DIR / "EDITING_INSTRUCTIONS.md").write_text(EDITING_INSTRUCTIONS)
    print(f"Wrote {DASHBOARD_DIR / 'EDITING_INSTRUCTIONS.md'}")
